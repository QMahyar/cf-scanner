...
- Frontend: one embedded HTML file; htmx + `htmx-ext-sse` 2.2.4; Pico.css
  2.1.1 (MIT, vendored); vanilla JS; zero build step
...
Release dry-run:  dist plan --artifacts=all --tag=v0.1.0
Release:          dist build --artifacts=all --tag=v0.1.0 && dist host ... (CI)
```
...
```
src/
  main.rs            CLI entry (clap): serve | scan | ranges
  server.rs          axum app: API routes, SSE, static frontend
  api/               request/response types (the contract all UIs share)
  engine/
    mod.rs           ScanController: orchestration, stop conditions, progress
    cdn.rs           phase-1 TCP/TLS scanner (tokio), ranges, ports
  ...
  util.rs
embed/
  index.html         single-file frontend (htmx + SSE + Pico)
data/
  cf-ranges.txt      official IPv4 CIDRs
  warp-pools.txt     known WARP endpoint pools
  geoip.mmdb         db-ip.com Lite Country+City (CC BY 4.0)
  xray-version.txt   pinned xray release tag
tests/               integration tests (engine + API)
docs/                intent, ADRs, README
tasks/               plan.md, todo.md
Cargo.toml           + [workspace.metadata.dist] (dist config)
.github/workflows/   release.yml (generated by dist), ci/checks.yml
```
...
- xray phase 2: integration test spawns a real local xray once (guarded by
  `CFSCANNER_TEST_XRAY=1` env and an available xray binary); skipped in CI
  until xray download is wired or marked `#[ignore]`.
