...
- [09: Test coverage push](tickets/09-test-coverage-push.md): E2E binary test (TEST-NET scan → NDJSON + CSV/JSON export file assertions incl. schema + atomic-write check); `--retry-last` error-path e2e; plan edges (/32 count=1, count>pool → Every); coverage gate 70→80 (measured locally with llvm-cov, passes). Landed as `5034b19`.
