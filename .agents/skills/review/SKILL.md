...
**Track progress.** Create a task for each step below, mark each in_progress when starting and completed when done. Check your task list after each step.
...
For **BIG CHANGE** reviews, delegate exploration to subagents.
...
Check installed skills for any that match the review's domain.

**Invoke matched skills now** — read their output and use domain guidance to inform your review.

For domains not covered by installed skills, search for a relevant skill.
...
When asking the user to choose, label each option with issue NUMBER and option LETTER. Recommended option is always first.
