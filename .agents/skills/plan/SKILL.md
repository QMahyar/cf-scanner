...
**Track progress.** Create a task for each step, mark each in_progress when starting and completed when done. Check your task list after completing each step.
...
Resolve `[[wikilinks]]` against `brain/` (the vault root): `[[principles/foundational-thinking]]` → `brain/principles/foundational-thinking.md`.

**Do NOT skip this. Do NOT use memorized principle content — always read fresh.**

## Step 2 — Define Scope and Constraints

Ask the user to resolve ambiguity before exploring the codebase:
...
Check installed skills for any that match the plan's domain. **Invoke matched skills** — read their output and incorporate domain guidance into the plan.

If the plan touches a domain **not covered** by installed skills, search for a relevant skill. If one is found, install it (project-local) and incorporate its guidance. Note what was installed so the user can see it. After the plan is written, delete any one-off skills that won't be needed again.
...
**Skill changes:** If a phase involves creating or updating a skill (any `SKILL.md` in a skills folder), the phase must instruct the implementer to use a skill-creation skill during that phase, if one is available.
